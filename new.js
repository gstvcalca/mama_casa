document.addEventListener("DOMContentLoaded", function () {
  const searchBar = document.querySelector('.sqs-block.search-block.sqs-block-search');
...
const request_quote_button = document.querySelector('button#request_quote_button');
if (request_quote_button){
  ...
  const designer = " by "+ currentProductName.parentElement.nextSibling.innerText;
...
       const formWrapper = doc.querySelector("div.form-wrapper");
      ...
      const submitButton = currentForm.querySelector('button.form-submit-button');
      const newDiv = doc.createElement('div');
     	const quoteTitleDiv = doc.createElement('div');
     	const quoteTitleP = doc.createElement('p');
    const productDiv = doc.querySelector('div#text-d0771f36-54bc-4b17-b31c-f308e09ea9bf');
      const productInput = productDiv.querySelector("input");
     	productDiv.style.display="none";
      // this function populate the product field 
      submitButton.addEventListener('mouseover', () => {
        productInput.value = productName.innerText + " - " + window.location.href;
        productInput.dispatchEvent(new Event('change', { bubbles: true }));
      });
  const signUpSpan = currentForm.querySelector('input[name="email-yui_3_17_2_1_1743181065641_1755-field"] + span');
  signUpSpan.innerText = 'Immerse yourself in Mama’s world with our newsletter!';
